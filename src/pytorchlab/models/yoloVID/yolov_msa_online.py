...
from src.pytorchlab.utils.yoloVID import bboxes_iou

from .losses import IOUloss
from .network_blocks import BaseConv, DWConv
from scipy.optimize import linear_sum_assignment
from src.pytorchlab.utils.yoloVID import box_cxcywh_to_xyxy, generalized_box_iou
from .post_trans import MSA_yolov_online
from torchvision.ops import roi_align
from .post_process import postprocess_pure as postprocess
from collections import namedtuple

Result = namedtuple('Result', "cls_feature reg_feature cls_scores reg_scores boxes msa")
...
class YOLOXHead(nn.Module):
    ...
    def forward(self, xin, other_result={}, labels=None, imgs=None, nms_thresh=0.5):
        ...
        if self.training:
            ...
            return self.get_losses(
                ...
                expanded_strides,
                labels,  # 16,120,5
                torch.cat(outputs, 1),  # 16,xxxx,12
                origin_preds,
                dtype=xin[0].dtype,
                refined_cls=fc_output,
                idx=pred_idx,  # 16,30,14
                pred_res=pred_result,  # 16,30,14
            )
        else:
            ...
            return result, Result(cls_feature=res_dict['cls_feature'], reg_feature=res_dict['reg_feature'],
                                  cls_scores=res_dict['cls_scores'], reg_scores=res_dict['reg_scores'],
                                  boxes=res_dict['boxes'], msa=res_dict['msa'])
    ...
    def get_losses(
        ...
        for batch_idx in range(outputs.shape[0]):
            ...
            if num_gt == 0:
                ...
                try:
                    (
                        gt_matched_classes,  # 匹配到的anchors的类别
                        fg_mask,  # 在中心或者在inbox的anchor的mask
                        pred_ious_this_matching,  # anchors的iou值
                        matched_gt_inds,  # 匹配到的GT编号
                        num_fg_img,  # 匹配到的anchors的个数
                ...
                cost_giou, iou = generalized_box_iou(pred_box, gt_xyxy)  # 和GTIOU前K高的anchors与FSM筛选后的anchors
                max_iou = torch.max(iou, dim=-1)
                for ele_idx, ele in enumerate(idx[batch_idx]):
                    loc = torch.where(fg_idx == ele)[0]

                    if len(loc):  # 如果匹配到，那就fg++，并且在ref——target对应位置上赋值
            ...
            ref_masks.append(ref_target[:, -1] == 0)  # 最后一列等于0说明有类别
            if self.use_l1:
                l1_targets.append(l1_target)

        cls_targets = torch.cat(cls_targets, 0)  # 每个batch都有与对应的target类别置信度
        reg_targets = torch.cat(reg_targets, 0)
        obj_targets = torch.cat(obj_targets, 0)
        ref_targets = torch.cat(ref_targets, 0)  # 经过FSM过后在经过IOU筛选过后的target
        ...
        x = ref_masks.sum().item()

        # refined_cls_num= refined_cls.view(-1, self.num_classes)
        if x > refined_cls.shape[1]:
            loss_ref = (  # 当前帧的预测结果与所有参考帧的结果取loss值
                           self.bcewithlog_loss(
                               refined_cls.view(-1, self.num_classes), ref_targets[ref_masks][:refined_cls.shape[1], :]
                           )
                       ).sum() / num_fg
        else:
            loss_ref = (  # 当前帧的预测结果与所有参考帧的结果取loss值
                           self.bcewithlog_loss(
                               refined_cls.view(-1, self.num_classes)[:x, :], ref_targets[ref_masks]
                           )
                       ).sum() / num_fg
    ...
    def get_assignments(
        ...
        # fg代表在中心或者在boxes之内的anchors 后者表示在box并且在center的anchors
        ...
        (
            num_fg,  # 匹配到的anchors个数
            gt_matched_classes,  # 分别匹配到的类别
            pred_ious_this_matching,  # 匹配到的IOU值
            matched_gt_inds,  # 匹配到的GT的编号
    ...
    def dynamic_k_matching(self, cost, pair_wise_ious, gt_classes, num_gt, fg_mask):
        ...
        if (anchor_matching_gt > 1).sum() > 0:  # 如果有某个anchor与多个GTIOU重合，去重
            _, cost_argmin = torch.min(cost[:, anchor_matching_gt > 1], dim=0)  # 获得最小cost的GT编号
            matching_matrix[:, anchor_matching_gt > 1] *= 0.0  # 归零
            matching_matrix[cost_argmin, anchor_matching_gt > 1] = 1.0  # 重新赋值
        fg_mask_inboxes = matching_matrix.sum(0) > 0.0  # 所有IOU值前K大的mask
        num_fg = fg_mask_inboxes.sum().item()
        # num =fg_mask.sum().item()
        fg_mask[fg_mask.clone()] = fg_mask_inboxes  # 只保留IOU值前K大的mask（K为动态变化的）
    ...
    def postprocess_single_img(self, prediction, num_classes, conf_thre=0.001, nms_thre=0.5):
        ...
        prediction_ori = [detect.detach() for detect in prediction]
